    ...
    def compute_fsp(g):
        ...
        for i in range(len(g) - 1):
            ...
            if b_H > t_H:
                ...
                pass
            bot = bot.unsqueeze(1)  # (N, 1, C, H, W)
            top = top.unsqueeze(2)  # (N, C, 1, H, W)
            bot = bot.view(bot.shape[0], bot.shape[1], bot.shape[2], -1)    # (N, 1, C, H*W)
            top = top.view(top.shape[0], top.shape[1], top.shape[2], -1)    # (N, C, 1, H*W)

            fsp = (bot * top).mean(-1)  # (N, C, C)
            fsp_list.append(fsp)
        return fsp_list
