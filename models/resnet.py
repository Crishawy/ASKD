...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.downsample is not None:  # 降采样
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, is_last=False):
        ...
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride,
                               padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv3 = nn.Conv2d(planes, planes * 4, kernel_size=1, bias=False)   # 输出通道为planes * expansion
...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, num_filters, block_name='BasicBlock', num_classes=10):
        ...
        self.inplanes = num_filters[0]  # layer1的输入通道
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):    # conv2d用kaiming normal初始化
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        downsample = None
        # 若输入通道与block的输入通道不等，则使用1x1 conv保持相等
        ...
        self.inplanes = planes * block.expansion    # 下一个block的输入通道
...
def resnet47(**kwargs):
    return ResNet(47, [16, 16, 32, 64], 'bottleneck', **kwargs)
