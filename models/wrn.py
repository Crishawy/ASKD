...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        if not self.equalInOut:     # preact:先bn和relu再conv
        ...
        out = self.conv2(out)
        # 通道相等则直接加x，不等则使用shortcut保持相等后加shortcut(x)
    ...
    def __init__(self, depth, num_classes, widen_factor=1, dropRate=0.0):
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                m.weight.data.normal_(0, math.sqrt(2. / n))     # 标准化
